...
if (!supabaseUrl || !serviceRoleKey) {
    console.error(
        "Missing Supabase configuration. Please set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in your environment.",
    );
    process.exit(1);
}
...
async function seedAvailability(ownerId: string, dogId: string) {
    const { error: cleanupError } = await supabase.from("availability").delete()
        .eq("owner_id", ownerId);
    if (cleanupError) {
        throw cleanupError;
    }
